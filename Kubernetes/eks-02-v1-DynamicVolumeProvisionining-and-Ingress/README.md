# Hands-on EKS-02-v1 : Dynamic Volume Provisionining and Ingress
...
- Part 1 - Updating aws-cli version 2 and Installing kubectl and eksctl on Amazon Linux 2
...
```bash
eksctl create cluster --region us-east-1 --zones us-east-1a,us-east-1b,us-east-1c --node-type t3a.medium --nodes 1 --nodes-min 1 --nodes-max 3 --name cw-cluster
```
...
```bash
eksctl utils associate-iam-oidc-provider --cluster cw-cluster --approve
```
...
- Check the StorageClass object in the cluster. 
...
> Copy the ingress-yaml-files folder to ec2-instance with `scp command`.

```bash
scp -i <pem-file> -r <ingress-yaml-files> ec2-user@<ec2-instance-public-ip>:~/ingress-yaml-files
```
...
- Go to the `ingress-yaml-files/to-do` directory and look at the contents.
